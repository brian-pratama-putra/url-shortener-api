...
from aplikasi.dao.api.db_dao import get_url_detail
from aplikasi.dao.api.redis_dao import cache_get


async def get_url_detail_controller(request):
    ...
    v_hasil     = await get_url_detail(v_short_code, v_user_id)

    if v_hasil["status"] == "T" and v_hasil["result"]:
        return await response_service(request, v_method, 200, 200, "Success", {"data": v_hasil["result"]})
    elif v_hasil["status"] == "T" and not v_hasil["result"]:
        return await response_service(request, v_method, 204, 200, "Data tidak ditemukan")
    else:
