...
from aplikasi.dao.api.db_dao import delete_url
from aplikasi.dao.api.redis_dao import cache_get, cache_delete
import asyncio


async def delete_url_controller(request):
    ...
    v_hasil     = await asyncio.to_thread(delete_url, v_short_code, v_user_id)

    if v_hasil["status"] == "T":
        await cache_delete(f"redirect:{v_short_code}")
        await cache_delete(f"url:detail:{v_short_code}")
        await cache_delete(f"url:list:{v_user_id}")
